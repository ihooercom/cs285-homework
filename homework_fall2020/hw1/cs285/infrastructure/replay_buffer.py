...
class ReplayBuffer(object):
    ...
    def sample_random_data(self, batch_size):
        assert (self.obs.shape[0] == self.acs.shape[0] == self.rews.shape[0] == self.next_obs.shape[0] ==
                self.terminals.shape[0])

        ## HINT 1: use np.random.permutation to sample random indices
        ## HINT 2: return corresponding data points from each array (i.e., not different indices from each array)
        ## HINT 3: look at the sample_recent_data function below
        random_idxes = np.random.permutation(np.arange(self.obs.shape[0]))
        sample_idxes = random_idxes[-batch_size:]

        return (self.obs[sample_idxes], self.acs[sample_idxes], self.rews[sample_idxes], self.next_obs[sample_idxes],
                self.terminals[sample_idxes])
