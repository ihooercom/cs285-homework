...
class RL_Trainer(object):
    ...
    def do_relabel_with_expert(self, expert_policy, paths):
        ...
        for i in range(len(paths)):
            bsize = self.params['train_batch_size']
            for j in range(0, len(paths[i]['observation']), bsize):
                ob_batch = paths[i]["observation"][j:j + bsize]
                ac_batch = expert_policy.get_action(ob_batch)
                paths[i]["action"][j:j + bsize] = ac_batch
