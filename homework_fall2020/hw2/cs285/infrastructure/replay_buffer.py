...
class ReplayBuffer(object):
    ...
    def add_rollouts(self, paths, noised=False):
        ...
        # self.paths = self.paths[-(self.max_size // 10):]
    ...
    def sample_random_data(self, batch_size):
        ...
        assert self.obs.shape[0] == self.acs.shape[0] == self.concatenated_rews.shape[0] == self.next_obs.shape[
            0] == self.terminals.shape[0]
    ...
    def sample_recent_data(self, batch_size=1, concat_rew=True):
        ...
        if concat_rew:
            return self.obs[-batch_size:], self.acs[-batch_size:], self.concatenated_rews[-batch_size:], self.next_obs[
                -batch_size:], self.terminals[-batch_size:]
        else:
