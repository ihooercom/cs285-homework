...
class CQLCritic(BaseCritic):
    ...
    def dqn_loss(self, ob_no, ac_na, reward_n, next_ob_no, terminal_n):
    ...
    def update(self, ob_no, ac_na, reward_n, next_ob_no, terminal_n):
        ...
        loss, qa_t_values, q_t_values = self.dqn_loss(ob_no, ac_na, reward_n, next_ob_no, terminal_n)

        # CQL Implementation
        # TODO: Implement CQL as described in the pdf and paper
        # Hint: After calculating cql_loss, augment the loss appropriately

        q_t_logsumexp = torch.logsumexp(qa_t_values)
